...
function inferQualityFromLinkColor(color)
	...
	--	lddebug("Colour-Quality --> "..color.." == "..sCol)
		if (color:lower()==sCol:lower()) then return n end
	end
...
local function TooltipUpdateHandler(control)
		...
		if (ld_tooltip.popupUptoDate) then return end;
		...
		if (control==PopupTooltip and control.lastLink == nil) then return end;
		
		
		if ld_tooltip.isNew==false or ld_tooltip.currLink==nil then return end --- Nothing to do
