...
function SaveSwitchTradingHouseGuild(name, guildId)
...
function gsse.PopulateGuildList()
	...
	--	local id,name,alliance = GetTradingHouseGuildDetails(i)
		...
		if not ZO_TradingHouse:IsHidden() then
			if gsse.canUseTradingHouse(id)==false then color="|cff0000" end
		else
			color="|cFFFF00"
		end
...
function gsse.nextTradingHouse(guildInd)
		...
		while guildInd<=gsse.MAX_GUILD_ID do
			...
			if gsse.canUseTradingHouse(guildInd) then return guildInd end
...
function SaveQueueTradingHouseSearch(name, params)
	local delay = GetTradingHouseCooldownRemaining()
...
function gsse.doTradingHouseSearch(name,params)
 ...
 function gsse.FindMatches()
    ...
    if searchTerms == "" then return end
...
function gsse.ShowItem(itemButton)
	...
	if resultPage<0 then return end;
...
function gsse.SetTooltip(itemData)
	...
	if gsse.data.itemData[itmIndex]==nil then --- no initial match
		...
		if gsse.data.itemData[itmIndex]==nil then return {} end
	end
